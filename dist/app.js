...
const UPLOADS_DIR = path_1.default.join(ROOT_DIR, 'uploads');
const PRODUCTS_UPLOAD_DIR = path_1.default.join(UPLOADS_DIR, 'products');
const BASE_URL = process.env.BASE_URL || `http://localhost:${PORT}`;
const imageService = new repository_1.ImageService();
// CORS configuration
const corsOptions = {
    origin: ['http://localhost:8080', 'http://localhost:3000', 'http://localhost:8081', 'https://diamondholdingsco.com', 'https://admin.diamondholdingsco.com'],
    credentials: true
};
// Initialize app
...
if (!fs_1.default.existsSync(PRODUCTS_UPLOAD_DIR)) {
    fs_1.default.mkdirSync(PRODUCTS_UPLOAD_DIR, { recursive: true });
...
const productService = new repository_1.ProductService();
const orderRepo = new repository_1.OrderRepository();
const orderDetailRepo = new repository_1.OrderDetailRepository();
const orderService = new repository_1.OrderService();
// ============ HELPER FUNCTIONS ============
...
app.get('/check-directories', (req, res) => {
    const directories = {
        ...
        UPLOADS_DIR: {
            path: UPLOADS_DIR,
            exists: fs_1.default.existsSync(UPLOADS_DIR),
            files: fs_1.default.existsSync(UPLOADS_DIR) ? fs_1.default.readdirSync(UPLOADS_DIR) : []
        },
        PRODUCTS_UPLOAD_DIR: {
            path: PRODUCTS_UPLOAD_DIR,
            exists: fs_1.default.existsSync(PRODUCTS_UPLOAD_DIR),
            files: fs_1.default.existsSync(PRODUCTS_UPLOAD_DIR) ? fs_1.default.readdirSync(PRODUCTS_UPLOAD_DIR) : []
        }
    };
...
app.get('/test-file/:filename', (req, res) => {
    try {
        ...
        const filePath = path_1.default.join(PRODUCTS_UPLOAD_DIR, filename);
...
app.get('/api/uploads', (req, res) => {
    try {
        const files = fs_1.default.readdirSync(PRODUCTS_UPLOAD_DIR);
        const fileInfo = files.map(file => {
            const filePath = path_1.default.join(PRODUCTS_UPLOAD_DIR, file);
            const stats = fs_1.default.statSync(filePath);
            return {
                filename: file,
